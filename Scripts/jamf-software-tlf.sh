...
setup_tlf_python() {
  local shared="/opt/edupy" py_prefix py_bin user group home link vscode settings cmd hash old_hash
  local packages=(numpy scipy pandas matplotlib scikit-learn xgboost catboost ipython jupyter jupyterlab ruff ipykernel)
  py_prefix="$(brew_admin --prefix "python@${PY_VER}")"
  py_bin="${py_prefix}/bin/python${PY_VER}"
  [[ -x "${py_bin}" ]] || fail "Cannot find ${py_bin}"

  if [[ -x "${shared}/bin/python" ]] && ! "${shared}/bin/python" -c \
      'import site,sys; assert sys.version_info[:2] == (3,14); assert site.ENABLE_USER_SITE' >/dev/null 2>&1; then
    [[ -f "${shared}/.theisland-managed" ]] || fail "${shared} is incompatible and not managed by this script"
    /bin/rm -rf "${shared}"
  fi
  if [[ ! -x "${shared}/bin/python" ]]; then
    [[ ! -e "${shared}" || -f "${shared}/.theisland-managed" ]] || fail "${shared} exists and is not managed by this script"
    /bin/rm -rf "${shared}"
    /usr/bin/install -d -o "${ADMIN_USER}" -g wheel -m 0755 "${shared}"
    run_logged "Creating shared Python environment" run_admin "${py_bin}" -m venv --system-site-packages "${shared}" || fail "Cannot create ${shared}"
    /usr/bin/touch "${shared}/.theisland-managed"
  fi

  hash="$(/usr/bin/printf '%s\n' "${packages[@]}" | /usr/bin/shasum -a 256 | /usr/bin/awk '{print $1}')"
  old_hash="$(/bin/cat "${shared}/.theisland-packages.sha256" 2>/dev/null || true)"
  if [[ "${hash}" == "${old_hash}" ]] && run_admin /usr/bin/env PIP_CONFIG_FILE=/dev/null \
      "${shared}/bin/python" -m pip show "${packages[@]}" >/dev/null 2>&1; then
    log "TLF Python packages already installed"
  else
    run_logged "Updating pip tooling" run_admin /usr/bin/env PIP_CONFIG_FILE=/dev/null \
      PIP_DISABLE_PIP_VERSION_CHECK=1 "${shared}/bin/python" -m pip install --no-input --upgrade \
      pip setuptools wheel || fail "Cannot update pip tooling"
    run_logged "Installing TLF Python packages" run_admin /usr/bin/env PIP_CONFIG_FILE=/dev/null \
      PIP_DISABLE_PIP_VERSION_CHECK=1 "${shared}/bin/python" -m pip install --no-input \
      "${packages[@]}" || fail "Cannot install TLF Python packages"
    /usr/bin/printf '%s\n' "${hash}" >"${shared}/.theisland-packages.sha256"
  fi

  /bin/cat >"${shared}/pip.conf" <<'EOF_PIP'
[global]
...
EOF_PIP
  ...
  /bin/mkdir -p /usr/local/bin /usr/local/share/jupyter/kernels/edupy /etc/paths.d
  /usr/bin/printf '%s\n' "${shared}/bin" >/etc/paths.d/00-edupy
  /usr/sbin/chown root:wheel /etc/paths.d/00-edupy
  /bin/chmod 0644 /etc/paths.d/00-edupy

  for cmd in python python3 "python${PY_VER}"; do
    /bin/cat >"/usr/local/bin/${cmd}" <<EOF_WRAP
#!/usr/bin/env bash
unset PYTHONHOME PYTHONNOUSERSITE
exec "${shared}/bin/python" "\$@"
EOF_WRAP
    /bin/chmod 0755 "/usr/local/bin/${cmd}"
  done
  for cmd in pip pip3 "pip${PY_VER}"; do
    /bin/cat >"/usr/local/bin/${cmd}" <<EOF_WRAP
#!/usr/bin/env bash
unset PYTHONHOME PYTHONNOUSERSITE
exec "${shared}/bin/python" -m pip "\$@"
EOF_WRAP
    /bin/chmod 0755 "/usr/local/bin/${cmd}"
  done
  for cmd in ipython jupyter jupyter-lab ruff; do
    /bin/cat >"/usr/local/bin/${cmd}" <<EOF_WRAP
#!/usr/bin/env bash
unset PYTHONHOME PYTHONNOUSERSITE
exec "${shared}/bin/${cmd}" "\$@"
EOF_WRAP
  ...
  /usr/sbin/chown root:wheel /usr/local/bin/python /usr/local/bin/python3 \
    "/usr/local/bin/python${PY_VER}" /usr/local/bin/pip /usr/local/bin/pip3 \
    "/usr/local/bin/pip${PY_VER}" /usr/local/bin/ipython /usr/local/bin/jupyter \
    /usr/local/bin/jupyter-lab /usr/local/bin/ruff

  /bin/cat >/usr/local/share/jupyter/kernels/edupy/kernel.json <<EOF_KERNEL
{"argv":["${shared}/bin/python","-m","ipykernel_launcher","-f","{connection_file}"],"display_name":"edupy","language":"python"}
EOF_KERNEL
  /usr/sbin/chown -R root:wheel /usr/local/share/jupyter/kernels/edupy
  /bin/chmod 0644 /usr/local/share/jupyter/kernels/edupy/kernel.json

  for user in "${LOCAL_USERS[@]}"; do
    home="/Users/${user}"
    group="$(/usr/bin/id -gn "${user}")"
    /bin/mkdir -p "${home}/.virtualenvs"
    link="${home}/.virtualenvs/edupy"
    if [[ -e "${link}" && ! -L "${link}" ]]; then
      log "WARNING: ${link} is not a symlink; leaving it unchanged"
    else
      /bin/rm -f "${link}"
      /bin/ln -s "${shared}" "${link}"
      /usr/sbin/chown -h "${user}:${group}" "${link}"
    fi
    /usr/sbin/chown "${user}:${group}" "${home}/.virtualenvs"
    vscode="${home}/Library/Application Support/Code/User"
    settings="${vscode}/settings.json"
    if [[ ! -e "${settings}" ]]; then
      /bin/mkdir -p "${vscode}"
      /bin/cat >"${settings}" <<EOF_VSCODE
{"python.defaultInterpreterPath":"${link}/bin/python","python-envs.globalSearchPaths":["${home}/.virtualenvs"]}
EOF_VSCODE
      /usr/sbin/chown -R "${user}:${group}" "${home}/Library/Application Support/Code"
    else
      log "VS Code settings already exist for ${user}; interpreter: ${link}/bin/python"
    fi
    write_user_path "${user}" 'export PATH="/opt/edupy/bin:/usr/local/bin:/opt/homebrew/bin:/opt/homebrew/sbin:${PATH}"'
  done

  /bin/chmod -RN "${shared}" 2>/dev/null || true
  /usr/sbin/chown -R "${ADMIN_USER}:wheel" "${shared}"
  /bin/chmod -R u+rwX,go-w "${shared}"
  for user in "${LOCAL_USERS[@]}"; do
    /usr/bin/sudo -n -u "${user}" -H /usr/local/bin/python -c \
      'import site,sys; assert sys.prefix == "/opt/edupy"; assert site.ENABLE_USER_SITE' >/dev/null 2>&1 || fail "Python verification failed for ${user}"
    /usr/bin/sudo -n -u "${user}" -H /usr/local/bin/pip config get install.user 2>/dev/null | \
      /usr/bin/grep -qx true || fail "pip configuration failed for ${user}"
  done
  log "Shared TLF Python environment ready at ${shared}"
}

log "START: TLF software profile"
list_users
[[ ${#LOCAL_USERS[@]} -gt 0 ]] || fail "TLF profile requires an existing personal student account"
check_common
...
setup_tlf_python
lock_homebrew
log "SUCCESS: TLF profile installed"
