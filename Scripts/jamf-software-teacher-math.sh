...
cleanup() {
  ...
  if [[ -n "${TEACHER_WORK_DIR}" && -d "${TEACHER_WORK_DIR}" ]]; then
    /bin/rm -rf "${TEACHER_WORK_DIR}" 2>/dev/null || true
  fi
...
acquire_lock() {
  ...
  if ! /bin/mkdir "${lock}" 2>/dev/null; then
    ...
    if [[ "${pid}" =~ ^[0-9]+$ ]] &&
      ...
      /bin/rm -rf "${lock}"
      /bin/mkdir "${lock}" ||
        fail "Cannot replace stale ${label} lock"
    else
...
detect_teacher() {
  ...
  home="$(
    /usr/bin/dscl . \
      -read "/Users/${TEACHER_USER}" NFSHomeDirectory \
      2>/dev/null |
  ...
  [[ -n "${home}" && -d "${home}" ]] ||
    fail "Cannot determine home directory for ${TEACHER_USER}"

  [[ "${home}" != *$'\n'* && "${home}" != *' '* ]] ||
    fail "Unsupported teacher home path: ${home}"
...
create_teacher_work_dir() {
  TEACHER_WORK_DIR="$(
    /usr/bin/mktemp \
      -d /private/var/tmp/theisland-schooltex-teacher.XXXXXX
  )"
...
check_prerequisites() {
  ...
  [[ -x /opt/homebrew/bin/python3.14 ]] ||
    fail "Python 3.14 is missing; run the general teacher profile first"
}
...
validate_brewfile() {
  /usr/bin/awk '
    ...
    /^[[:space:]]*cask_args[[:space:]]+appdir:[[:space:]]*"\/Applications"[[:space:]]*$/ {
      next
    }
    /^[[:space:]]*(brew|cask)[[:space:]]+"[A-Za-z0-9@+._-]+"[[:space:]]*$/ {
      next
    }
    {
...
fetch_managed_files() {
  ...
  sha="$(
    /usr/bin/shasum -a 256 "${PACKAGES_FILE}" |
      /usr/bin/awk '{print $1}'
  )"
...
prepare_homebrew_for_admin() {
  ...
  [[ "${owner}" == "${ADMIN_USER}" ||
     "${owner}" == "${TEACHER_USER}" ]] ||
    fail "Unexpected Homebrew owner: ${owner}"
...
restore_homebrew_to_teacher() {
  SERVICES_COMMAND="$(
    brew_admin command services 2>/dev/null ||
      true
  )"
  ...
  /usr/bin/sudo \
    ...
    /bin/test \
    -w "${BREW_PREFIX}" ||
    fail "${TEACHER_USER} cannot write to Homebrew"
}
...
reconcile_homebrew() {
  ...
  if run_logged \
    "Checking math-teacher Brewfile" \
    brew_admin bundle check \
      --file="${BREWFILE}" \
      --no-upgrade; then
    ...
    run_logged \
      ...
      brew_admin update-if-needed ||
      fail "brew update-if-needed failed"

    run_logged \
      "Applying math-teacher Brewfile" \
      brew_admin bundle \
        --file="${BREWFILE}" \
        --no-upgrade ||
    ...
    run_logged \
      "Verifying math-teacher Brewfile" \
      brew_admin bundle check \
        --file="${BREWFILE}" \
        --no-upgrade ||
      fail "Math-teacher Brewfile remains incomplete"
  fi
...
select_texlive_repository() {
  ...
  for candidate in "${TEXLIVE_REPOSITORY_CANDIDATES[@]}"; do
    ...
    if [[ -n "${excluded_repository}" &&
          "${final_repo}" == "${excluded_repository}" ]]; then
  ...
  fail "No responsive TeX Live ${TEXLIVE_YEAR} repository found"
}
...
verify_texlive_year() {
  ...
  version="$(
    run_teacher "${TEXBIN}/tlmgr" --version 2>&1
  )" || return 1

  /usr/bin/printf '%s\n' "${version}" |
    /usr/bin/grep -Eq \
      "TeX Live.*${TEXLIVE_YEAR}"
}
...
install_texlive_if_needed() {
  ...
  /bin/mkdir -p \
    "${TEACHER_HOME}/Library/texmf" \
    "${SCHOOLTEX_ROOT}/texmf-local"

  /usr/sbin/chown \
    -R "${TEACHER_USER}:${TEACHER_GROUP}" \
    "${SCHOOLTEX_ROOT}" \
    "${TEACHER_HOME}/Library/texmf"
  ...
  curl_file \
    ...
    "${archive}" \
    >>"${LOG_FILE}" 2>&1 ||
    fail "Cannot download install-tl"

  curl_file \
    "${TEXLIVE_REPOSITORY}/install-tl-unx.tar.gz.sha512" \
    "${checksum}" \
    >>"${LOG_FILE}" 2>&1 ||
    fail "Cannot download install-tl SHA-512"

  [[ -s "${archive}" && -s "${checksum}" ]] ||
    fail "Downloaded install-tl files are empty"
  ...
  actual="$(
    /usr/bin/shasum -a 512 "${archive}" |
      /usr/bin/awk '{print $1}'
  )"

  [[ "${expected}" =~ ^[0-9a-fA-F]{128}$ &&
     "${actual}" == "${expected}" ]] ||
    fail "install-tl SHA-512 verification failed"
  ...
  for candidate in \
    "${extract_dir}"/*/install-tl \
    "${extract_dir}"/install-tl; do
  ...
  [[ -n "${installer_path}" ]] ||
    fail "Cannot locate install-tl after extraction"

  installer_dir="$(
    /usr/bin/dirname "${installer_path}"
  )"
  ...
  run_teacher /bin/test -r "${profile}" ||
    fail "install-tl profile is not readable by ${TEACHER_USER}"

  while (( install_attempt <= 3 )); do
    if run_logged \
      "Installing TeX Live ${TEXLIVE_YEAR} scheme-small from ${TEXLIVE_REPOSITORY} (attempt ${install_attempt}/3)" \
    ...
    failed_repository="${TEXLIVE_REPOSITORY}"

    log "WARNING: install-tl failed using ${failed_repository}"
    log "Removing incomplete TeX Live tree before retry"

    /bin/rm -rf "${TEXDIR}"

    select_texlive_repository "${failed_repository}"

    log "Retrying install-tl with ${TEXLIVE_REPOSITORY}"
...
reconcile_packages() {
  if ! repository_is_current_year; then
    log "WARNING: selected TeX Live repository stopped responding; selecting another mirror"
    select_texlive_repository "${TEXLIVE_REPOSITORY}"
  fi

  load_packages

  run_logged \
    "Setting TeX Live repository" \
    run_teacher \
      "${TEXBIN}/tlmgr" \
      option repository \
      "${TEXLIVE_REPOSITORY}" ||
  ...
  run_logged \
    "Updating tlmgr infrastructure" \
    run_teacher \
      "${TEXBIN}/tlmgr" \
      update --self ||
    fail "tlmgr self-update failed"

  run_logged \
    "Installing SchoolTeX package manifest" \
    run_teacher \
      "${TEXBIN}/tlmgr" \
      install "${PACKAGES[@]}" ||
    fail "tlmgr package installation failed"
...
install_latex_workshop() {
  ...
  [[ -x "${code_cli}" ]] ||
    fail "Visual Studio Code CLI is missing"
...
merge_vscode_settings() {
  ...
  /usr/sbin/chown \
    -R "${TEACHER_USER}:${TEACHER_GROUP}" \
    "${TEACHER_HOME}/Library/Application Support/Code"
  ...
  if ! run_teacher \
    ...
    /usr/bin/install \
      ...
      -m 0644 \
      "${VSCODE_SETTINGS_FILE}" \
      "${settings_dir}/settings.schooltex.json"
...
verify_installation() {
  ...
  marker_matches ||
    fail "SchoolTeX managed marker is missing or invalid"

  verify_texlive_year ||
    fail "tlmgr does not report TeX Live ${TEXLIVE_YEAR}"
  ...
  /usr/bin/sudo \
    ...
    -H \
    /bin/test \
    -w "${TEXDIR}/tlpkg/texlive.tlpdb" ||
    fail "${TEACHER_USER} cannot update TeX Live"
  ...
  [[ -x /Library/TeX/texbin/latexmk ]] ||
    fail "/Library/TeX/texbin is not usable"
